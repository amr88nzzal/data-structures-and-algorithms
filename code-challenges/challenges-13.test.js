...
const longestString = (arr) => {
  // Solution code here...
  const longStringA = arr.reduce((acc, cur) => acc.length < cur.length ? acc = cur : acc, '');
  return arr.indexOf(longStringA);
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
 return arr.map(x=>x.split('').filter(x=>/[0-9]/.test(x)).join(''));
};
...
const onlyOddChars = (str) => {
  // Solution code here...
  return str.split('').filter((value,i)=>i%2!==0&&value).join('');
};
...
const allHappy = (arr) => {
  // Solution code here...
  return arr.filter(value=>value.includes(':)')).length===arr.length;
};
