...
const sortByChildren = (charArray) => {
  // Solution code here...
  charArray.sort((strOne, strTwo) => {
    return strOne.children.length - strTwo.children.length;
  });
  return charArray;
};
...
const containsW = (str) => {
  // Solution code here...
  const pattern = /[w]/g;
  const filter = str.match(pattern);
  if (filter) {
    return true;
  }
  else {
...
const citiesAtoJ = (arr) => {
  ...
  let pattern = /[A-J]/;
  let newArr = [];
  arr.map(result => (pattern.test(result[0])) ? newArr.push(result) : '');
  return newArr;
};
