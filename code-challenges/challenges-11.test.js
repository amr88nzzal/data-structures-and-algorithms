...
const count = (target, input) => {
  // Solution code here...
  return input.reduce((acc, res) => res.filter(value => value === target).length + acc, 0);
};
...
const totalSum = (input) => {
  // Solution code here...
  let sum = 0;
  input.map(res => res.map(value => sum += value));
  return sum;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(resArr => resArr.filter(value => ((typeof value) === 'number') && (value % 5 === 0)).map(value => Math.pow(2, value)));
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  return data.filter(result => (result.gender).toLowerCase() === 'male'.toLowerCase() || (result.gender).toLowerCase() === 'female'.toLowerCase()).map(result => result.name).join(' and ');
};
...
let findShortest = (data) => {
  // Solution code here...
  return data.reduce((acc, currentValue) => Number(acc.height) < Number(currentValue.height) ? acc : currentValue).name;
};
