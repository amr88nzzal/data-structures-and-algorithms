...
const maxInArray = (arr) => {
  // Solution code here...
  let result = [];
  result = arr.reduce((accumulator, currentValue) => Math.max(accumulator, currentValue));
  return result;
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
...
const getHouses = (arr) => {
  let houses = [];
  // Solution code here...
  Object.values(arr).map(result => houses.push(result.house));
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
  ...
  for (let result of arr) {
    if (Object.values(result).includes(character)) {
      return result.children && result.children.length;
    }
  }
